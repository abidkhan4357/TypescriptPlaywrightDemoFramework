  ...
  private async addToCart(product: Product) {
    if (product.quantity > 1) {
      for (let i = 0; i < product.quantity; i++) {
        ...
        await this.addToCartButton().waitFor({ state: 'visible', timeout: 30000 }).catch(async () => {
          console.log('Add to cart button not visible, hovering over product image again...');
          await this.productImage().hover();
